...
create_plot <- function(data, x_title, y_title, height, width) {
  ggplot(data, aes(x=TimeTreatColocRelationship, y=Percentage, fill=ColocRelationship)) +
    ...
    theme(text=element_text(family="Arial Black "),
          ...
          legend.text = element_text(size=20, margin = margin(r = 1, l=1, unit = 'cm')), #margin keeps the key with the text together, while "leg>
          legend.position="top",
          legend.justification="center",
          legend.margin=margin(t=1,l=1,b=1., unit="cm"),
  ...
  server <- function(input, output) {
    ...
    output$downloadPlot <- downloadHandler(
      ...
      content = function(file) {
        plot <- create_plot(data(), input$x_title, input$y_title, as.integer(input$height), as.integer(input$width))
        ggsave(file, plot, device = "png", height = as.integer(input$height) / 100, width = as.integer(input$width) / 100, dpi = 150)
        file.rename("plot.png", file)

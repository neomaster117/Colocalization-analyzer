...
create_plot <- function(data, x_title, y_title, height, width) {
  ggplot(data, aes(x=TimeTreatColocRelationship, y=Percentage, fill=ColocRelationship)) +
    ...
    theme(text=element_text(family="Arial Black "),
          ...
          legend.text = element_text(size=20, margin = margin(r = 1, unit = 'cm')), #margin keeps the key with the text together, while "legend.spacing.x = unit(2, 'cm')" would space everything apart 2cm
          legend.position="top",
